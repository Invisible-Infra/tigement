...
router.get('/owned', async (req: AuthRequest, res) => {
  try {
    if (!(await requirePremium(req, res))) return;
    const result = await query(
      `SELECT id, source_table_id, encrypted_table_data, version, wrapped_dek_for_owner, updated_at
       FROM shared_tables WHERE owner_id = $1 ORDER BY updated_at DESC`,
      [req.user!.id]
    );
    res.json({ shares: result.rows });
  } catch (error) {
  ...
  try {
    ...
    const result = await query(
      `SELECT st.id, st.source_table_id, st.version, st.created_at, st.updated_at,
              json_agg(json_build_object('userId', r.user_id, 'email', u.email, 'permission', r.permission)) as recipients
...
// PATCH /api/shares/:id - Update permission or revoke (id = shared_table_recipients.id or shared_table_id)
...
router.patch('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    const { permission, revokeUserId } = updateShareSchema.parse(req.body);
...
router.put('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    await query(
      'UPDATE shared_tables SET encrypted_table_data = $1, version = $2, updated_at = NOW() WHERE id = $3',
      [encryptedTableData, version, id]
    );
